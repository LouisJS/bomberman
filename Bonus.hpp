...
// Last update Sun Jun  9 17:36:28 2013 bigott-a
//
...
{
public:
  Bonus(float xt, float yt, float zt, float xr, float yr, float zr, enum Type type);
  virtual ~Bonus();
  void		initialize(void);
  int		update(gdl::GameClock const &, gdl::Input &, std::list<AObject*> &);
  void		draw(void);
  enum Genre	getTypeObject() const { return (BONUS); }
